podTemplate(containers:[
  ...
  node(POD_LABEL) {
    stage("Compilacion") {
        container('gradle'){
            script{
			   git url: 'https://github.com/ASG-BPM/cosmonaut-orquestador',branch:'main',credentialsId: 'winter_user'
			   tag = sh(script:'git describe --tags --always `git rev-list --tags` | grep PROD | head -1',returnStdout: true ).trim()
			   sh "git checkout $tag"
			   sh "cd nomina-engine && gradle assemble -Ptag=$tag"
		   }
        }
     }
     stage("Push de imagen") {
        container('docker-registry'){
            script{
				sh "/kaniko/executor -f `pwd`/nomina-engine/Dockerfile -c `pwd` --cache=true --destination=gcr.io/cosmonaut-uat/cosmonaut/orquestador:$tag"
			}
        }
     }
        ...
        container('container-registry'){
            script{
		      ...
		      sh "kubectl  --insecure-skip-tls-verify get pods"
		       sh "sed -i 's/\$TAG/$tag/' nomina-engine/src/jenkins/cosmonaut-orquestador_deployment-qa.yml"
			   sh "cat nomina-engine/src/jenkins/cosmonaut-orquestador_deployment-qa.yml | kubectl --insecure-skip-tls-verify  apply -f -"  
